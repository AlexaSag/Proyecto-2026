...
#wklsModal{
    ...
    right:auto;    /* 🔥 evita expansión */
    display:none;
}

#wklsModal .ffi-content{
    display:inline-block;     /* 🔥 clave para que NO se estire */
    
    width:fit-content !important;
    min-width:unset !important;   /* 🔥 rompe el 550px */
    max-width:none !important;
...
#wklsModal button:hover{
    background:#3a3a3c;
}
#wklsModal input[type="checkbox"]{
    accent-color:#0a84ff;   /* 🔥 azul moderno */
    transform:scale(1.1);
}
...
#icModal .ffi-content{
    ...
    width:fit-content;
    max-width:320px;   /* puedes ajustar */
    padding:12px;
}
.wkls-item{
    display:flex;
...
<body>
<div class="top-bar">
    <button class="wkls-toggle" onclick="toggleWKLS()">WKLS</button>
...
<h1>S.A.🧠</h1>
<!------       cheker MODAL       ------>
<div id="wklsModal" class="ffi-modal">
  <div class="ffi-content">
    <span class="close-btn" onclick="closeWKLS()">✖</span>
    <h2>WKLS</h2>

    <button onclick="openWKLS('frp')">FRP</button>
    <button onclick="openWKLS('cwf')">CWF</button>

    <div id="wklsContent" style="margin-top:10px;"></div>
...
<button id="payNA" onclick="setPayment('NA new account')">NA</button>
    <br><br>
...
<div id="monthsContainer">
    ...
    <div class="ffi-row">
        ...
        <input type="text" class="bankInput" placeholder="Bank/Account Holder/Number">
        <br><br>
        <strong>Ending Balance:</strong>
        <div class="month-row">

            <div class="month-box">
                <button onclick="this.nextElementSibling.click()">Jan</button>
                <select class="monthPicker" onchange="setMonthFromPicker(this)"></select>
                <input type="text" class="monthAmount" placeholder="$"
                oninput="formatMoney(this)"
                onkeypress="return event.charCode >= 48 && event.charCode <= 57">
            </div>

            <div class="month-box">
                <button onclick="this.nextElementSibling.click()">Feb</button>
                <select class="monthPicker" onchange="setMonthFromPicker(this)"></select>
                <input type="text" class="monthAmount" placeholder="$"
                oninput="formatMoney(this)"
                onkeypress="return event.charCode >= 48 && event.charCode <= 57">
            ...
            <div class="month-box">
                <button onclick="this.nextElementSibling.click()">Mar</button>
                <select class="monthPicker" onchange="setMonthFromPicker(this)"></select>
                <input type="text" class="monthAmount" placeholder="$"
                oninput="formatMoney(this)"
                onkeypress="return event.charCode >= 48 && event.charCode <= 57">
    ...
    <!-- 🔥 BOTÓN FUERA -->
    <button type="button" id="addRowBtn" onclick="addMonth()">+</button>
...
////////////////////////////////////////////////////
////                GLOBAL BUTTON FEEDBACK  TEMPLATES
////////////////////////////////////////////////////
...
function closeAllTemplates(){
    ...
    document.querySelectorAll(".ffi-modal").forEach(modal=>{
...
noteToggle.onclick = function(){
    ...
    closeAllTemplates(); // 🔥 cierra templates
...
function openTemplate(id){
    ...
    closeAllTemplates(); // 🔥 siempre limpia antes
...
function setPayment(value){
    ...
    payYes.classList.remove("proxy-green","proxy-red","proxy-yellow");
    payNo.classList.remove("proxy-green","proxy-red","proxy-yellow");
    payNA.classList.remove("proxy-green","proxy-red","proxy-yellow");

    if(value==="Yes") payYes.classList.add("proxy-green");
    if(value==="No") payNo.classList.add("proxy-red");
    if(value==="NA new account") payNA.classList.add("proxy-yellow");
}
...
let amounts = row.querySelectorAll(".monthAmount");
let buttons = row.querySelectorAll(".month-box button");

let monthsText = Array.from(amounts).map((input, i)=>{

    let short = buttons[i].innerText;
    let index = monthsShort.indexOf(short);
    let full = index !== -1 ? monthsFull[index] : "";
    let val = input.value.replace(/[^0-9.]/g, "");

    return val ? `${full}: $${val}` : null;
...
.join(", ");
    ...
    if(ending || monthsText){
    endingSection.push(`$${ending}${monthsText ? ", " + monthsText : ""}`);
}
...
Bank/Account Holder/Number: ${bankSection.join(" | NEXT BANK INFO: | ")}
Ending Balance: ${endingSection.join(" | NEXT STATEMENT: | ")}
...
window.addEventListener("DOMContentLoaded", ()=>{
    ...
    let today = new Date().getMonth();   // mes actual (0–11)
    let start = (today - 2 + 12) % 12;   // 🔥 2 meses atrás
    ...
    document.querySelectorAll(".month-row").forEach(row=>{
        ...
        buttons.forEach((btn, i)=>{
            let mIndex = (start + i) % 12;
            btn.innerText = monthsShort[mIndex];
        });
    });
    fillMonthPickers(); // 🔥 ESTA ES LA CLAVE
});
...
function resetFFI(){
    ...
    // 🔥 limpiar meses
document.querySelectorAll(".monthAmount")
.forEach(i => i.value = "");
...
payNA.classList.remove("proxy-yellow");
    proxyYes.classList.remove("proxy-green");
    proxyNo.classList.remove("proxy-red");
    proxySelfEmployee.classList.remove("proxy-yellow");
...
<strong>Next Bank info:</strong>
<input type="text" class="bankInput" placeholder="Bank/Account Holder/Number">

<strong>Ending:</strong>
<div class="month-row" data-start="0">
<br><br>
    <div class="month-box">
        <button onclick="this.nextElementSibling.click()">Jan</button>
        <select class="monthPicker" onchange="setMonthFromPicker(this)"></select>
...
<button type="button" onclick="removeRow(this)" class="remove-btn">x</button>
`;

    container.insertBefore(row, button);
    fillMonthPickers(); // 🔥 IMPORTANTE
}
...
function updateRemoveButtons(){
    const rows = document.querySelectorAll("#monthsContainer .ffi-row");
...
/////////////////////////////////////////////////////
////               MONTH SYSTEM 🔥.  edicion del original 
////////////////////////////////////////////////////
...
function selectMonth(index, btn){

    let row = btn.closest(".month-row");
    let boxes = row.querySelectorAll(".month-box");

    // guardar mes base
    row.dataset.start = index;

    boxes.forEach((box, i)=>{
        let mIndex = (index + i) % 12;
        let button = box.querySelector("button");

        button.innerText = monthsShort[mIndex];
    });

    // highlight
    row.querySelectorAll("button").forEach(b=>b.classList.remove("active-month"));
...
// 🔹 abrir selector al dar click en botón
function openMonthPicker(btn){
    let box = btn.closest(".month-box");
    let select = box.querySelector(".monthPicker");

    select.style.opacity = "1";   // mostrar
...
// 🔹 cuando eliges mes
function setMonthFromPicker(select){

    let index = parseInt(select.value);
    if(isNaN(index)) return;

    let box = select.closest(".month-box");
    let button = box.querySelector("button");

    button.innerText = monthsShort[index];
}
function fillMonthPickers(){

    document.querySelectorAll(".monthPicker").forEach(select=>{
...
function formatMoney(input){
    ...
    // convertir a número con decimales (últimos 2 dígitos)
...
////////////////////////////////////////////////////
////          FUNCTION    MAPS / STREET VIEW  TEMPLATES
////////////////////////////////////////////////////
...
////////////////////////////////////////////////////
function toggleWKLS(){
    closeAllTemplates(); // 🔥 importante
    document.getElementById("wklsModal").style.display="flex";
}

function closeWKLS(){
    document.getElementById("wklsModal").style.display="none";
}
const wklsData = {
    frp: ["Iniciar","Analizar","Procesar","Pagar","Listar","Cerrar"],
    cwf: ["Iniciar","Analizar","Procesar","Pagar","Listar","Cerrar"]
};
function openWKLS(type){

    let tasks = wklsData[type];

    let html = tasks.map((t,i)=>`
        <label style="display:block; margin:5px 0;">
            <input type="checkbox" onchange="checkWKLS(this)"> ${t}
        </label>
    `).join("");

    html += `<div id="wklsDone" style="margin-top:10px;font-weight:600;"></div>`;

    document.getElementById("wklsContent").innerHTML = html;
}
function checkWKLS(){

    let checks = document.querySelectorAll("#wklsContent input[type='checkbox']");
    let allChecked = [...checks].every(c => c.checked);

    let result = document.getElementById("wklsDone");

    if(allChecked){
        result.innerText = "✅ Tarea terminada";
...
function openWKLS(type){
    ...
    let html = tasks.map((t,i)=>`
    <div class="wkls-item">
        <input type="checkbox" onchange="checkWKLS(this)">
        <span>${t}</span>
    </div>
`).join("");

    html += `
        <div id="wklsDone" style="margin-top:10px;font-weight:600;"></div>

        <div style="margin-top:10px;">
            <button id="wklsResetBtn" onclick="resetWKLS()">Reset</button>
        </div>
    `;
...
function resetWKLS(){
    ...
    // desmarcar todos
    document.querySelectorAll("#wklsContent input[type='checkbox']")
    .forEach(c => c.checked = false);

    // limpiar mensaje
    document.getElementById("wklsDone").innerText = "";

    // feedback visual (usa tu sistema global 🔥)
    resetFeedback(document.getElementById("wklsResetBtn"));
}

////////////////////////////////////////////////////////////////////////

////////////////////////////////////////////////////
////                     FUNCTION CALCULATOR TEMPLATES 
////////////////////////////////////////////////////
